...
require('http').createServer((req,res)=>{
  ...
  var arr=req.url.split('/').filter(e=>e!='');
  arr.forEach((e,i,a)=>a[i]=decodeURIComponent(e));
  if(arr.length>1&&arr.includes('-i')){
    arr.push('-fs');
    arr.push('300000000');
    arr.push('pipe:1');
  }
  const out=(require('child_process').spawnSync)(require('@ffmpeg-installer/ffmpeg').path,arr).output;
  if(out[1].toString('utf8')!=''){
    res.write(out[1]);
    return res.end();
  }
  res.writeHead(200,{'Content-Type':'text/plain'});
  res.write('+--------------------------------------------+\n|             FFmpeg on the Web!             |\n| https://'+req.headers.host+'/param/param2/etc. |\n|    Use https://uguu.se for local files.    |\n+--------------------------------------------+\n\nWARNING: RAM IS LOW, SO PROBABLY DON\'T USE IN THE FIRST PLACE.\n\n');
  res.write(out[2]);
  res.end();
}).listen(8080);
