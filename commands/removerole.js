...
module.exports.run = async (bot, message, args) => {
  ...
  if (!rMember.roles.cache.get(role.id)) {
    return message.channel.send(`**__${rMember.displayName}__**, already doesn't have the role!`).then(m => {
      m.delete(10000)
    });
  } else {
    await rMember.cache.removeRole(role.id).catch(e => console.log(e.message))
