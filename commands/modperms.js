...
module.exports.run = async(bot, message, args) => {
    ...
    let botembed = new Discord.MessageEmbed()
        ...
        .setColor("#0e2b82")
        .setDescription("`addrole = MANAGE_ROLES ` | `ban = BAN_MEMBERS` | `kick = KICK_MEMBERS` | `mute = MANAGE_ROLES` | `purge = MANAGE_MESSAGES` | `removerole = MANAGE_ROLES` | `strike = KICK_MEMBERS` | `unban = BAN_MEMBERS` | `unmute = MANAGE_ROLES` | `lockdown = MANAGE_MESSAGES` | `commend = MANAGE_MESSAGES` | `slowmode = MANAGE_MESSAGES` | `createemoji = MANAGE_EMOJIS` | `setprefix = ADMINISTRATOR` | `giveawaystart = MANAGE_MESSAGES` | `giveawayreroll = MANAGE_MESSAGES` | `giveawayend = MANAGE_MESSAGES` | `setlogs = MANAGE_CHANNELS` | **All other commands are available to everyone!**")
